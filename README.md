# ResQWay ⚡
### Predictive Logistics Intelligence Platform for North East India

[![Rails Tests](https://img.shields.io/badge/Rails%20Tests-345%20Passing-emerald?style=flat-square&logo=ruby)](test/)
...
> *"In high-altitude terrain, supply chains can fail before vehicles even depart. When a landslide strikes NH-13 or the Sela Pass freezes over, logistics managers need predictive intelligence — knowing which corridors are at risk, which settlements face isolation, which staging depots have critical stock, and how relief convoys can navigate dynamically."*

---

## What is ResQWay?

Across the 8 states of North East India (Arunachal Pradesh, Assam, Manipur, Meghalaya, Mizoram, Nagaland, Sikkim, and Tripura), complex mountain geography and intense monsoon seasons create severe logistics challenges. Cloudbursts, flash floods, and rockfalls frequently sever vital supply corridors without warning.

**ResQWay** is an end-to-end **Predictive Logistics Intelligence Platform** engineered specifically for the North Eastern Region of India. Built for the **Smart India Hackathon (SIH 2026)**, it turns environmental telemetry, terrain topography, road quality indices, and live field reports into anticipatory logistics intelligence.

While ResQWay serves daily smart freight coordination and accessibility monitoring, it includes a dedicated **Emergency Response & Disaster Command Module** that activates during crisis events:
...
5. **Human-in-the-Loop Emergency Command**: Provides decision-support optimization, multi-depot split dispatch, and plan drift detection without ever replacing human authority.
...
ResQWay's interface is inspired by the clean, readable editorial aesthetic of **ChaiCode** (`dsa.chaicode.com`):
...
| **Testing** | Rails Minitest (345 tests, 100% passing), Pytest (7 tests, 100% passing) |
...
```bash
git clone https://github.com/priyanshukun46/resqway.git
cd resqway
```
...
| Role | Email | Password |
| :--- | :--- | :--- |
| **Administrator** | `admin@resqway.ai` | `password123` |
| **Field Operator** | `operator@resqway.ai` | `password123` |
...
- **`/routes`** — Test the multi-criteria route optimization algorithm between any two Northeast hubs.

---

## Running the Tests

We take reliability seriously in mission-critical logistics tech. Both the Rails monolith and Python ML services maintain full test coverage:

```bash
# 1. Run all Rails model, controller, and service tests
bin/rails test
...
│   ├── services/            # Domain logic and ResQWay:: intelligence services
...
└── test/                    # Full Minitest suite (345 tests, 0 failures)
```
